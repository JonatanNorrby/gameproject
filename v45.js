...
// the exact supplied Main Base sprite when that asset is present in the repo.
...
(function installV45MainBaseSprite(){
  ...
  const CANDIDATES=[
    'assets/main_base.jpeg?v=45',
    'assets/main_base.jpg?v=45',
    'main_base.jpeg?v=45',
    'D23468BC-D610-4A00-80DE-BC868A553231.jpeg?v=45'
  ];

  const img=new Image();
  img.decoding='async';
  let ready=false,processed=null,candidateIndex=0;

  function backgroundPixel(d,i){
    const r=d[i],g=d[i+1],b=d[i+2];
    const lo=Math.min(r,g,b),hi=Math.max(r,g,b);
    // JPEG source has a white background. Only near-neutral, near-white pixels
    // CONNECTED TO AN IMAGE EDGE are removed; bright armour panels inside the
    // actual Base therefore remain untouched.
  ...
  function tryCandidate(){
    if(candidateIndex>=CANDIDATES.length){
      console.warn('v45: Main Base sprite not found in repository. Expected assets/main_base.jpeg; keeping procedural Base fallback.');
      return;
    }
  ...
  img.onload=()=>{
    if(img.naturalWidth!==SOURCE_W||img.naturalHeight!==SOURCE_H){
      console.warn(`v45: ignoring Main Base candidate with unexpected dimensions ${img.naturalWidth}x${img.naturalHeight}; expected ${SOURCE_W}x${SOURCE_H}.`);
      tryCandidate();return;
    }
    try{processed=removeConnectedWhiteBackground(img);ready=true;window.__apdMainBaseSpriteReady=true;}
    catch(err){console.error('v45: could not prepare supplied Main Base sprite',err);processed=null;ready=false;tryCandidate();}
  };
  ...
  window.__apdMainBaseSprite={
    expectedPath:'assets/main_base.jpeg',sourceWidth:SOURCE_W,sourceHeight:SOURCE_H,
    crop:{...CROP},worldWidth:WORLD_W_PX,worldHeight:WORLD_H_PX,
    sourceIsJpeg:true,runtimeBackgroundMask:true
  };
})();
