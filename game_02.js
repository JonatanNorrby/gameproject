...
function findSafeSpotAt(x,y){
 return state.safeSpots.find(s=>!s.occupied&&Math.hypot(x-s.x,y-s.y)<=CONFIG.SAFE_SPOT.RADIUS);
}
function releaseSafeSpotForTower(t){
 if(!t.safeSpotId)return;
 const s=state.safeSpots.find(v=>v.id===t.safeSpotId);
...
function removeDeadTowers(){
 ...
 for(const t of state.towers){
   ...
   else releaseSafeSpotForTower(t);
 }
...
canvas.addEventListener("click",e=>{
 ...
 if(selected==="safespot"){
   ...
   state.credits-=cost;updateHud();return;
 }
 ...
 if(selected==="blockade"){
   ...
   state.towers.push({type:"soldier",x,y,range:def.range,hp,maxHp:hp,protected:protectedTower,safeSpotId,
     memberCooldowns:Array(CONFIG.SOLDIER.MEMBERS).fill(0).map(()=>Math.random()*.3)});
 ...
 state.credits-=cost;updateHud();
});

function stageTheme(){
 const themes=[
  {ground:"#10221b",patch:"#234a37",path:"#18372d",accent:"#59d49a",sky:"#0b1512"},
  {ground:"#241529",patch:"#4a2852",path:"#35203d",accent:"#d47cff",sky:"#160d19"},
  {ground:"#261a12",patch:"#51321f",path:"#3b2718",accent:"#ff9c51",sky:"#17100b"},
  {ground:"#101d29",patch:"#234058",path:"#182e40",accent:"#64c6ff",sky:"#0b1219"}
 ];
 return themes[(state.stage-1)%themes.length];
}
function launchWave(localWave){
 if(localWave>STAGE_WAVES)return;
 const absoluteWave=(state.stage-1)*STAGE_WAVES+localWave;
 state.waveSpawners.push({localWave,absoluteWave,remaining:CONFIG.SPAWN.BASE_ENEMIES_PER_WAVE+absoluteWave*CONFIG.SPAWN.EXTRA_ENEMIES_PER_ABSOLUTE_WAVE,timer:.04});
 els.message.textContent=`Stage ${state.stage}, wave ${localWave} launched. Active waves: ${totalActiveWaves()}`;
 updateHud();
}
...
function weightedRarity(){
 ...
 for(const r of ["common","uncommon","rare","epic"]){sum+=RARITIES[r].weight;if(x<sum)return r}
 return "common";
}
function pickUpgrade(){
 for(let tries=0;tries<20;tries++){
  const rarity=weightedRarity(),pool=UPGRADES.filter(u=>u.rarity===rarity);
  if(pool.length)return pool[Math.floor(Math.random()*pool.length)];
 }
 return UPGRADES[0];
}
function showCards(){
 state.choosing=true;const choices=[];const used=new Set();
 while(choices.length<3){
  const up=pickUpgrade();
  if(!used.has(up.name)){used.add(up.name);choices.push(up)}
 }
 els.cards.innerHTML="";
 choices.forEach(up=>{
  ...
  c.onclick=()=>{up.apply(state);state.choosing=false;els.overlay.classList.add("hidden");els.message.textContent=`${up.rarity.toUpperCase()} upgrade: ${up.name}`;advanceIfStageComplete();updateHud()};
...
function shootFrom(x,y,e,type,damage,speed,angleJitter=0){
 let a=Math.atan2(e.y-y,e.x-x)+(Math.random()-.5)*angleJitter;
 state.bullets.push({x,y,vx:Math.cos(a)*speed,vy:Math.sin(a)*speed,dmg:damage,life:1.8,type});
}
